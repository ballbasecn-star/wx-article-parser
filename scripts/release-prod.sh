...
WX_ARTICLE_PARSER_IMAGE="${WX_ARTICLE_PARSER_IMAGE:-ballbase/wx-article-parser}"
DEPLOY_HOST="${DEPLOY_HOST:-}"
DEPLOY_USER="${DEPLOY_USER:-root}"
DEPLOY_PASSWORD="${DEPLOY_PASSWORD:-}"
DEPLOY_PORT="${DEPLOY_PORT:-22}"
DEPLOY_BASE_DIR="${DEPLOY_BASE_DIR:-/root/apps/parsers/wx-article-parser}"
WX_ARTICLE_PARSER_HOST_PORT="${WX_ARTICLE_PARSER_HOST_PORT:-8000}"
SERVER_HEALTHCHECK_URL="${SERVER_HEALTHCHECK_URL:-http://127.0.0.1:${WX_ARTICLE_PARSER_HOST_PORT}/api/v1/health}"
SERVER_CAPABILITIES_URL="${SERVER_CAPABILITIES_URL:-http://127.0.0.1:${WX_ARTICLE_PARSER_HOST_PORT}/api/v1/capabilities}"
...
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  run_step \
    ...
    WX_ARTICLE_PARSER_IMAGE="${WX_ARTICLE_PARSER_IMAGE}" \
