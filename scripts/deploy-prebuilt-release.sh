...
if [[ ! -f "${BUNDLE_DIR}/images/wx-article-parser.tar" ]]; then
...
"${SCP_CMD[@]}" "${BUNDLE_DIR}/images/wx-article-parser.tar" "${DEPLOY_USER}@${DEPLOY_HOST}:${DEPLOY_BASE_DIR}/images/wx-article-parser.tar"

echo "==> 在服务器加载镜像并启动服务"
"${SSH_CMD[@]}" "docker load -i '${DEPLOY_BASE_DIR}/images/wx-article-parser.tar' && \
